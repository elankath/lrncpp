...
=== std::invoke
A generic helper for writing generic code that calls a callable, regardless of whether
it is a function, a lambda, a operator or a member function.
[source,cpp]
----
...
The `foo` class below is quite DRY, because all the data is stored in immutable shared pointers which automatically know
how to copy themselves around. All member functions of `foo` should be const.
